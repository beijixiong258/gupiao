...
当前工作区的建库验收快照采用上述低额度模式：截至 2026-08-03，已保存 2022-01-04 至 2026-07-31 共 1108 个完整交易日、5,808,898 行全市场日线和 5535 只最新上市股票资料快照，日线覆盖率为 100%。该仓库使用 `daily.pre_close` 在读取时推导前复权链；历史 `daily_basic` 仍从空缺状态渐进积累，估值、规模和换手因子缺失时由模型训练窗筛选排除。
...
历史仓库建好后，日常不需要再填写日期。每个交易日收盘后执行下面的命令，程序会从仓库最后一个完整日线的下一天开始，只补到最近应完成的交易日，并固定使用低额度价格模式：

```powershell
gpyj warehouse update
```

开盘前或仓库已经是最新时，该命令直接返回 `up_to_date`，不会重复下载完整日期。它只提供程序内的安全增量入口，不会擅自在 Windows 中创建计划任务；如需无人值守运行，可以再由用户自行决定是否接入 Windows“任务计划程序”。

低额度账号还可以在额度允许时执行下面的命令。它每次只补一个最近缺失交易日的 `daily_basic`，不会一次扫完整段历史；重复执行会从新到旧逐日积累估值、流通市值和换手率：

```powershell
gpyj warehouse enrich
```
...
单股第二阶段预测和板块预测成功输出时，程序会把当时的股票、信号日、期限、点收益、成本后收益、上涨概率、区间、验证状态和推荐状态冻结到 `agent\cache\prediction_ledger.sqlite3`。记录写入后不会用未来结果修改原预测；流水账启用前的历史预测也不会倒填伪造。

每次执行 `gpyj warehouse update` 后，程序会用新写入的真实日线自动揭晓已经到期的记录。也可以随时执行：

```powershell
gpyj performance
gpyj performance --window 60
gpyj performance --kind single
gpyj performance --kind board
```

默认同时报告近20、60、120个目标交易日的方向准确率、MAE、RMSE、平均预测/实际成本后收益、80%区间覆盖率和概率Brier分数，并把单股、板块、推荐候选和三个期限分开统计。少于20个独立信号日时只标记为记录不足；达到更多信号日也只表示事前跟踪样本逐渐成熟，不会自动宣称模型已经有效。
...
板块选股未说明数量时默认请求 Top 8，单批最多返回 8 只模型推荐；如果用户明确要求超过 8 只，程序先说明单批上限，再正常返回第一批 Top 8。用户说“不满意、换一批、继续”时，程序使用稳定序列编号顺延到第 9～16 名，之后继续顺延，不重复前一批。只有满足执行约束且加权成本后模型收益为正的股票才进入 `recommended_candidates`；其余股票保留在 `model_ranking` 中供核对，不称为推荐。每只推荐候选都返回 T+1、T+2、T+3 成本前后收益预测；没有周期通过验证时仍可返回正收益模型候选，同时标明低可信度。
...
日K收益预测使用绝对误差梯度提升树与稳健缩放Ridge的小型集成，并用独立的正则Logistic模型估计上涨方向概率。板块选股另用 `XGBRanker`：每个交易日作为一个 `qid`，按同日股票未来成本后收益的分档排名学习推荐顺序；收益模型只负责预测板块中性超额收益，再与独立预测的市场/板块基准收益合成为总收益。输入除个股前复权日K外，还包括上证/沪深300/中证1000日K、同行或板块宽度与相对收益，以及Tushare可用时按同一交易日精确匹配的历史估值、流通市值和换手因子；缺失历史估值不会向前填充。程序还用当日可见的沪深300近20日趋势、市场宽度和20日波动率生成弱市/震荡市/强市状态分数、状态标记和趋势×宽度交互项，让统一模型学习不同市场环境，不直接拆成样本过少的三个小模型。每个训练窗口独立完成流通市值中性化、因子稳定筛选和去极值，验证数据不能参与这些边界的拟合。全市场日频仓库覆盖所需区间后，程序自动用本地仓库读取这些数据并扩大横截面，否则保持原来的小面板和远程降级链。

方向概率使用只依赖更早日期的滚动交叉拟合；只有保序校准同时优于原始概率和滚动历史上涨比例的Brier分数时才启用。收益区间额外训练10%/50%/90%分位数模型，并优先使用滚动CQR校准后的80%区间；样本不足时自动退回对称滚动共形或近邻经验区间。单股每个滚动折只能用更早折的样本外结果选择集成权重，板块外层验证权重只从训练期内部尾窗选择。训练样本默认同时比较不衰减、一年半衰期和两年半衰期；候选只在当前训练窗的净化尾部比较，且达到最低相对MAE改善才给新样本更高权重，否则继续等权。所选权重同时用于点收益、方向概率、分位数区间、板块基准和板块排序。收益模型统一与零收益、训练中位数和训练均值三个朴素基准比较；板块排序模型另报告 NDCG@3、NDCG@8、Rank IC 和 Top-N 成本后收益。板块结果把 `selection_confidence`（推荐顺序可信度）与 `return_confidence`（收益数值可信度）分开显示；验证失败时仍公开模型预测点值，并把验证结果作为可信度标签。
...
- `lianghua_peizhi.json`：单股同行池、日频仓库覆盖后扩容上限、滚动验证、证据评估门槛，以及板块模型历史天数、股票数量、过滤规则、最佳朴素基准、弃权门槛、时间衰减、动态滑点/成交参与率、树/Ridge集成参数和 T+1/T+2/T+3 权重。
...
- `moxing.time_decay_enabled`、`time_decay_half_life_candidates`：是否允许时间衰减及候选交易日半衰期，`0` 表示等权；
- `moxing.time_decay_calibration_ratio`、`time_decay_min_relative_improvement`、`time_decay_min_weight`：训练窗尾部选择比例、启用衰减所需最低相对MAE改善和旧样本权重下限；
...
单股分析需要建立同行历史面板并完成六折滚动验证和分位数区间训练，通常需要 1 到 6 分钟。板块选股会先从 SQLite 仓库一次批量读取多只股票，只对缺失股票逐只联网，再完成六个样本外稳定子窗和分位数区间训练，通常需要 1 到 6 分钟；数据源重试、限频或可用股票较多时会更久。
...
- 单股模型优先读取不晚于分析日的本地股票资料快照来确定同行，再用少量全市场高流动性股票补足，使用六折扩展窗口滚动验证；没有历史快照时会明确披露当前成分偏差。
...
- 板块工具优先使用通过样本外验证的周期排名；如果没有周期通过，则自动使用全部 T+1、T+2、T+3 模型估计排序。只有加权成本后收益为正且可执行的股票进入低可信度推荐名单；没有正收益候选时返回空推荐并保留完整 `model_ranking`。
