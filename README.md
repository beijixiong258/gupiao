...
智能体会把“深科技”识别为 `000021.SZ`，把“持有两个交易日”严格映射到 **T+2**，然后调用单股分析工具。程序先确认分析时间、完整收盘日、实时快照来源和可交易性，再用同行优先的高流动性 A 股历史面板训练 T+1/T+2/T+3 模型。T+2 只有通过三段滚动样本外验证（最后一段为保留测试窗口）、扣除交易成本后仍有优势且股票可执行时，才可能给出“倾向买入”；否则会返回“等待”“回避”或“证据不足”。程序不会连接券商或替你下单。
...
涉及“现在、今天、当前、最新、能不能买”或明确持有交易日的提问，会强制在本轮重新调用研究工具，不复用旧会话行情。程序升级前保存的旧版单股工具结果和它生成的文字结论会自动标记为过期；不需要手动删除会话，也不需要用 `/new` 规避。如果模型仍试图直接回答，程序会拦截并要求它重新调用工具；再次绕过时，本轮会明确失败，不会展示或保存可能过期的结论。
...
单股工具现在同时输出基本面、技术面、可交易性和 T+1/T+2/T+3 同行面板预测。预测的是下一交易日开盘入场到指定可卖出收盘的收益，并展示扣费后期望、经验上涨比例、经验区间和验证状态；由于实际开盘价尚未知，程序不会伪造精确目标价。板块选股仍要求明确板块名称，不会无范围地扫描并推荐整个 A 股市场。
...
- `lianghua_peizhi.json`：单股同行池、滚动验证、决策门槛，以及板块模型历史天数、股票数量、过滤规则、样本外验证门槛和 T+1/T+2/T+3 权重。
- `jiaoyi_chengben.json`：佣金、最低佣金、过户费、印花税和滑点。

单股分析和板块选股都会读取并校验这些量化参数；数据源不在 JSON 中设置，应使用每次命令的 `--source`。预测周期只能是 T+1、T+2、T+3。程序会拒绝启用自动交易相关配置；无论配置如何，本程序都没有券商连接和下单工具。
...
- 单股问题中的“持有两个交易日”严格使用 T+2，不再由技术指标启发式打分代替期限模型。
- 盘中询问时，实时价只用于检查成交与涨停状态。如果最近完整收盘信号对应的次日开盘已经过去，程序返回“等待”，不会把盘中追价伪装成模型计划入口。
- 单股模型优先选取当前同行股票，再用少量全市场高流动性股票补足，使用三段扩展窗口滚动验证；当前成分回看历史仍存在幸存者偏差，结果会明确披露。
- 单股结论固定为“倾向买入、等待、回避、证据不足”之一。可交易性、指定期限验证、合法买入数量或成本后优势任一不通过，都不会给出“倾向买入”。
...
- 实际 T+1 开盘前无法可靠换算目标收盘价，因此 `predicted_close` 返回空值。单股结果给出的最高可接受入场价是防追高失效线，不是预测成交价或目标价。
