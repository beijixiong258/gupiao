...
模型只在所需历史区间至少有 60 个交易日、且日线、复权因子和 `daily_basic` 的完整交易日覆盖率达到 90% 时使用仓库扩容：单股同行池由最多 20 只扩大到 60 只，板块历史池由 24 只扩大到 80 只。覆盖不足时自动保持原规模并继续使用现有缓存/联网降级链，不会拿残缺仓库强行训练。`gpyj settings` 和 `gpyj warehouse status` 都能查看仓库状态。

仓库从第一次同步起按实际抓取日保存上市、退市、暂停上市股票资料快照，不会把今天的行业标签倒填成历史标签。因此，日线、复权、估值、规模和换手可以按交易日回放，但仓库建立前的历史行业/板块成分仍无法凭空恢复，幸存者偏差只能减轻，不能宣称完全消除。所有数据和模型仍严格限定为日 K，不含任何 30/60 分钟数据。
