...
“能不能买”按扣除广义交易费用后是否仍有上涨空间理解；“能不能卖”先分析剩余上涨空间，如果缺少持仓信息，分析完成后会再询问买入价和股数或持仓金额，以便计算净盈亏。费用覆盖佣金及最低佣金、过户费、卖出印花税、基础双边滑点、成交额参与率与 ATR 动态冲击，以及合法申报数量。指定周期未通过样本外验证或预测时点已经失效时，第二阶段不会把内部原始点预测展示给用户。
...
方向概率只在按时间后切的样本外窗口确实改善Brier分数时做保序校准；80%区间同时报告滚动共形覆盖诊断。单股每个滚动折只能用更早折的样本外结果选择集成权重，板块外层验证权重只从训练期内部尾窗选择。模型统一与零收益、训练中位数和训练均值三个朴素基准比较，并要求优于其中表现最好的基准。输出保留实验指纹、折间稳定性、市场状态分层、模型权重和组件分歧，但验证失败时仍不会公开内部预测点值。
...
- `moxing.abstain_min_net_return`、`abstain_min_positive_probability`、`abstain_min_quality_score`：发布研究候选前的扣费收益、上涨概率和样本外质量门槛；
...
- 板块工具只有通过样本外验证、Top-N 扣成本收益为正且优于最佳朴素基准的周期才参与排名；最终候选还必须达到成本后收益、校准上涨概率和质量分门槛，否则明确返回“弃权”。
- 交易成本按目标资金、个股价格、所属板块合法买入数量、最新成交额参与率和 ATR 动态冲击计算；资金不足或测算仓位超过成交容量上限时会明确列为执行约束。
- 单股和板块结果中的 `signal_gate.decision` 会明确给出 `research_candidate` 或 `abstain`，并列出未通过的验证、执行、收益、概率或质量条件；“弃权”表示程序不发布研究候选，不是卖出指令。
