...
当前工作区的建库验收快照采用上述低额度模式：截至 2026-07-23，已保存 2022-01-04 至 2026-07-23 共 1102 个完整交易日、5,775,745 行全市场日线和 5530 只股票资料快照，日线覆盖率为 100%。该仓库使用 `daily.pre_close` 在读取时推导前复权链；历史 `daily_basic` 仍从空缺状态渐进积累，估值、规模和换手因子缺失时由模型训练窗筛选排除。
