...
每个交易日会分别保存全市场未复权日线、复权因子和 `daily_basic`；其中某个接口失败时，下次只重试未完成部分。需要从旧日期向后补，或一次处理更多交易日时可以执行：
...
`--max-sessions 0` 表示处理范围内全部缺口，可能运行很久并触发数据源限频，日常更适合分批执行。交易日尚未收盘或数据源尚未更新时，当天会保持失败/待补状态，收盘后重新执行即可。仓库保存在 `agent\cache\a_share_daily_warehouse.sqlite3`，不会提交到 Git，但会随历史增长持续占用本机磁盘。

模型只在所需历史区间至少有 60 个交易日、且日线与一致复权链的完整交易日覆盖率达到 90% 时使用仓库扩容：单股同行池由最多 20 只扩大到 60 只，板块历史池由 24 只扩大到 80 只。`daily_basic` 覆盖率单独报告；缺失的估值、规模和换手特征不会填造，由每个训练窗的因子覆盖与稳定性筛选决定是否剔除。价格覆盖不足时自动保持原规模并继续使用现有缓存/联网降级链。`gpyj settings` 和 `gpyj warehouse status` 都能查看仓库状态。

仓库从第一次同步起按实际抓取日保存上市、退市、暂停上市股票资料快照，不会把今天的行业标签倒填成历史标签。标准模式取得的估值、规模和换手可以按交易日回放；低额度模式只保证日线与复权链，不把缺失因子说成已有。仓库建立前的历史行业/板块成分仍无法凭空恢复，幸存者偏差只能减轻，不能宣称完全消除。所有数据和模型仍严格限定为日 K，不含任何 30/60 分钟数据。
...
日K预测使用梯度提升树与稳健缩放Ridge的小型集成，并用独立的正则Logistic模型估计上涨方向概率。输入除个股前复权日K外，还包括上证/沪深300/中证1000日K、同行或板块宽度与相对收益，以及Tushare可用时按同一交易日精确匹配的历史估值、流通市值和换手因子；缺失历史估值不会向前填充。每个训练窗口独立完成流通市值中性化、因子稳定筛选和去极值，验证数据不能参与这些边界的拟合。全市场日频仓库覆盖所需区间后，程序自动用本地仓库读取这些数据并扩大横截面，否则保持原来的小面板和远程降级链。

方向概率只在按时间后切的样本外窗口确实改善Brier分数时做保序校准；80%区间同时报告滚动共形覆盖诊断。单股每个滚动折只能用更早折的样本外结果选择集成权重，板块外层验证权重只从训练期内部尾窗选择。输出保留实验指纹、折间稳定性、市场状态分层、模型权重和组件分歧，但验证失败时仍不会公开内部预测点值。
...
- `lianghua_peizhi.json`：单股同行池、日频仓库覆盖后扩容上限、滚动验证、证据评估门槛，以及板块模型历史天数、股票数量、过滤规则、样本外验证门槛、树/Ridge集成参数和 T+1/T+2/T+3 权重。
